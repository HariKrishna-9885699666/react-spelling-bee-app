...
import { IWORD_POOL } from "../data/iwords";

const WORDS_PER_PAGE = 50;

export default function IWordListScreen({ onBack }: { onBack: () => void }) {
  const [page, setPage] = useState(0);
  const totalPages = Math.ceil(IWORD_POOL.length / WORDS_PER_PAGE);
  const wordsToShow = IWORD_POOL.slice(page * WORDS_PER_PAGE, (page + 1) * WORDS_PER_PAGE);

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-blue-50 p-4">
      <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-2xl">
        <h2 className="text-2xl font-bold text-blue-700 mb-4 text-center">Word List 2</h2>
